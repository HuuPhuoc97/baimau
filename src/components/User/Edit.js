...
import { setTitleAction, postAction ,allcategories} from 'actions'
...
class Edit extends Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(
            setTitleAction('Edit New Post', 'Home / Blog / Edit New Post')
        )
        ...
        this.props.dispatch(getPostAction(postId)).then((data) =>{
            if(!data.value.error){
                ...
                this.setState({
                    ...
                    photo: post.photo,
                    info: post.info,
                    categories: post.categories && post.categories[0] ? post.categories[0].id : 1
                })
                console.log('khoi tao edit', this.state);   
            }
    ...
    submitForm = e => {
        ...
        this.props.dispatch(postAction(data)).then(({ value, action }) => {
            if (!value.error) {
                this.setState({
    ...
    render() {
        ...
        // console.log('aaa',post); return null ;
        if (post && post.title && this.state.title == '') {
                            ...
                            <div className="avata">
                                ...
                                <div className="photo">
                                    <Upload
                                        ...
                                        value= {this.state.photo ? this.state.photo.name : null}
                                    >
                                    {imageUrl ? (
                                            <img src={imageUrl} alt="avatar" />
                                        ) : 
                                            post && post.photo ? <img src={API_URL + post.photo.url} alt="avatar" /> : uploadButton
                                        }
                                        ...
                                        >
                                            {cate.map(item => (
                                                <option
                                                    value={item.id}
                                                    key={item.id}
                                                    checked={item.id === this.state.categories}
                                                >
                                                    {item.name}
                                                </option>
                                            ))}
